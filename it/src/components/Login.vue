...
<v-responsive class="mx-auto px-5" max-width="300">
    ...
    <v-form v-model="form" @submit.prevent="login">
        <v-text-field :readonly="loading" :rules="[required]" clearable class="my-5" label="Username" type="text" hint="Enter your username to login" v-model="username"></v-text-field>

        <v-text-field :readonly="loading" :rules="[required]" clearable label="Password" type="password" hint="Enter your password to login" v-model="password"></v-text-field>

        <div id="formButtons">
            <v-btn :disabled="!form" :loading="loading" type="submit" class="mx-2" id="btnPrimary" depressed large>Login</v-btn>

            <v-btn to="/register" type="submit" id="btnSecondary" depressed large>Register</v-btn>
        </div>
    </v-form>
