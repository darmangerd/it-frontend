...
<v-responsive class="mx-auto px-5">
    ...
    <v-form v-model="form" @submit.prevent="register">
        <v-text-field :readonly="loading" :rules="usernameRules" clearable class="my-5" label="Username" type="text" hint="Enter your username to register" v-model="username"></v-text-field>

        <v-text-field :readonly="loading" :rules="passRules" clearable label="Password" type="password" hint="Enter your password to register" v-model="password"></v-text-field>

        <div class="text-caption mt-5">Weight {{ weight }} kg</div>
        <v-slider v-model="weight" :min="40" :max="150" step="10" :thumb-size="15" color="rgb(66, 199, 140)" show-ticks="always"></v-slider>

        <div class="text-caption">Height {{ height }} cm</div>
        <v-slider v-model="height" :min="100" :max="230" step="10" :thumb-size="15" color="rgb(66, 199, 140)" show-ticks="always"></v-slider>
        ...
        <div id="formButtons">
            <v-btn class="mx-2" id="btnSecondary" depressed large to="/">Login</v-btn>
            <v-btn type="submit" :disabled="!form" :loading="loading" id="btnPrimary" depressed large>Register</v-btn>
        </div>
